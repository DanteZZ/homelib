    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title className="htext">Поиск книг</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row>
            <Col>
          ...
          <Row>
            ...
            {!imageLoading && list.length !== 0 && (
              <>
                ...
                {list?.[nav]?.results?.length === 0 && (
                  <Alert className="mt-2">Книг не найдено</Alert>
                )}
                {list?.[nav]?.results?.length !== 0 &&
                  list[nav].results.map((i, k) => (
                    <Card
                      className="mt-2 external-product"
                      key={k}
                      onClick={() => setData(i)}
                    >
                      <Card.Body>
